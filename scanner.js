...
function scanAlias(cutoff, existing) {
  var results = [];
  // Search for forwarded Alias emails (from psngoldie) OR direct alias emails
  var threads = GmailApp.search(
    '(from:info@alias.org OR from:psngoldie@gmail.com subject:"Just Sold") after:' + dateStr(cutoff),
    0, 50
  );
  threads.forEach(function(thread) {
    thread.getMessages().forEach(function(msg) {
      ...
      // Only process Alias sale emails
      if (!/just sold/i.test(subj) && !/alias/i.test(body)) return;

      // Shoe name from subject: "Fwd: Your Nike Wmns Air Force 1 '07 'Triple White' Just Sold..."
      var shoeName = "";
      var subjM = subj.match(/Your\s+(.+?)\s+Just Sold/i);
      if (subjM) shoeName = subjM[1].trim();
      if (!shoeName) shoeName = subj.replace(/^Fwd?:\s*/i,"").replace(/Just Sold.*/i,"").replace(/^Your\s+/i,"").trim();

      // Price: "Price: $79"
      var salePrice = null;
      var spM = body.match(/^Price:\s*\$?([\d,]+\.?\d*)/im);
      if (spM) salePrice = parseFloat(spM[1].replace(/,/g,""));

      // Payout: "Amount Made: $64.20"
      var payout = null;
      var pyM = body.match(/Amount Made:\s*\$?([\d,]+\.?\d*)/i);
      if (pyM) payout = parseFloat(pyM[1].replace(/,/g,""));
      ...
      var orderM = body.match(/Order #:\s*([A-Za-z0-9\-]+)/i);
      var orderId = orderM ? orderM[1].trim() : "";
